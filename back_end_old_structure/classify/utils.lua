...
function write_raw_predictions(dump_dir, y_hat_frame)
  os.execute('mkdir -p ' .. dump_dir)
  ...
  out_fid = io.open(paths.concat(dump_dir, 'full_pred.txt'), 'w')
  for i=1,#y_hat_frame do
    for j=1,y_hat_frame[i]:size(1) do
      out_fid:write(tostring(y_hat_frame[i][j][1]) .. ' ' .. tostring(y_hat_frame[i][j][2]) .. ' ' .. tostring(y_hat_frame[i][j][3]) .. ' ' .. tostring(y_hat_frame[i][j][4]) .. '\n')
    end
    out_fid:write('\n')
    out_fid:write('\n')
  end
