...
if not opt then
   ...
   cmd:option('-data_type', 'multi', 'the type of the data: dummy | vot | neg_vot | multi')
   cmd:option('-test', 'classify_test.t7', 'the path to the test data')
   cmd:option('-input_dim', 63, 'the input size')
   cmd:option('-model_path', '../results/multi_natalia_drop_0.3/model.net', 'the path to the model') 
...
--[[-- getting durations
y = calc_onset_offset_neg(y_frame, true)
y_hat = calc_onset_offset_neg(y_hat_frame, false)

-- plot stats
plot_classification_stats_neg_vot(y, y_hat)
]]--

-- write predictions to file
write_raw_predictions(opt.dump_dir, y_hat_frame)
