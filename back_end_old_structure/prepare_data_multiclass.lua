...
function build_test_dataset(path, labels_filename, names_filename)    
  ...
  for file in lfs.dir(path) do
    ...
    if string.sub(file, #file-3, -1) == ".txt" then 
      ...
      if #labels[file] == 2 then
        y[{{labels[file][1], labels[file][2]}, 1}] = 2
        y[{{labels[file][2], dim}, 1}] = 3
      elseif #labels[file] == 3 then
        y[{{labels[file][1], labels[file][2]}, 1}] = 1
        y[{{labels[file][2], labels[file][3]}, 1}] = 2
        y[{{labels[file][3], dim}, 1}] = 3
      end        
...
function build_dataset(path, labels_filename, names_filename)    
  ...
  for file in lfs.dir(path) do    
    ...
    if string.sub(file, #file-3, -1) == ".txt" then  
      ...
      if #labels[file] == 2 then
        y[{{y_count+labels[file][1], y_count+labels[file][2]}, 1}] = 2
        y[{{y_count+labels[file][2], tmp_x_count-1}, 1}] = 3
      elseif #labels[file] == 3 then
        y[{{y_count+labels[file][1], y_count+labels[file][2]}, 1}] = 1
        y[{{y_count+labels[file][2], y_count+labels[file][3]}, 1}] = 2
        y[{{y_count+labels[file][3], tmp_x_count-1}, 1}] = 3
      end        
      x_count = tmp_x_count
      y_count = y_count+ dim
      idx = idx + 1
...
if not opt then
   ...
   cmd:text()
   cmd:text('Options:')
   cmd:option('-input_dir', '/Users/yossiadi/Datasets/vot/natalia/features/multi_vot/train/', 'the path to the dir where the data files are placed (in a .txt format)')
   cmd:option('-output_file', 'data/multi_class/train.t7', 'the path to save the t7 binary file')
