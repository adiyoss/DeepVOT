...
function build_dataset(path, labels_filename, names_filename)    
  ...
  for file in lfs.dir(path) do    
    ...
    if string.sub(file, #file-3, -1) == ".txt" then  
      ...
      y[{{y_count+labels[file][1], tmp_x_count-1}, 1}] = 1
...
if not opt then
   ...
   cmd:text()
   cmd:text('Options:')
   cmd:option('-input_dir', '/Users/yossiadi/Datasets/vot/dmitrieva/neg_vot/measurement/test/', 'the path to the dir where the data files are placed (in a .txt format)')   
   cmd:option('-output_file', 'data/neg_vot/test_neg_measurement.t7', 'the path to save the t7 binary file')   
   ...
   cmd:option('-is_test', false, 'boolean flag which indicates of we build the train or test dataset')
   cmd:text()
