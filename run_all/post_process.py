...
def post_process(prob_file, output_path):
    ...
    for n0 in range(N_min[0], min(N_max[0], N - 3)):
        for n1 in range(n0 + N_min[1], min(n0 + N_max[1], N - 2)):
            for n2 in range(n1 + N_min[2], min(n1 + N_max[2], N - 1)):
    ...
    # without prevoicing
    for n0 in range(N_min[0], min(N_max[0], N - 3)):
        for n2 in range(n0 + N_min[2], min(n0 + N_max[2], N - 1)):
